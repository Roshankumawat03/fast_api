...
def login_user(data: Annotated[OAuth2PasswordRequestForm, Depends()], db: SessionDep):
    ...
    if not user_data:
        return HTTPException(401, "Give Username Not Found In The DB.")
    
    password_check = bcrypt.checkpw(data.password.encode("UTF-8"), user_data.password)
...
validation_login = OAuth2PasswordBearer(tokenUrl="login")