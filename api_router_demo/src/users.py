...
def get_all_users(db : SessionDep, login_info: Annotated[str, Depends(validation_login)]):
...
def update(username, login_info: Annotated[str, Depends(validation_login)], db : SessionDep, data = Body(...)):
...
@user_routes.delete("/delete_by_username/{username}")
def delete(username, login_info: Annotated[str, Depends(validation_login)], db : SessionDep):
    ...
    if username != login_info:
        return HTTPException(409, "You are not allowed to deleted any other user.")
